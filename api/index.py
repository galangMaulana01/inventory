...
# AI product search — NVIDIA NV-CLIP embeddings (gratis/dev tier).
# Kalau NVIDIA_API_KEY belum diset, fitur AI nonaktif dengan pesan yang jelas
# (bukan crash), sisanya (inventori inti) tetap jalan normal.
NVIDIA_API_KEY = os.getenv("NVIDIA_API_KEY")
NVIDIA_EMBED_URL = "https://integrate.api.nvidia.com/v1/embeddings"
NVCLIP_MODEL = os.getenv("NVCLIP_MODEL", "nvidia/nvclip")
AI_CONFIDENT_SCORE = float(os.getenv("AI_CONFIDENT_SCORE", "0.85"))
AI_CONFIDENT_GAP = float(os.getenv("AI_CONFIDENT_GAP", "0.05"))
...
def get_products():
    ...
    return [p for p in load_json("products", {"items": []}).get("items", []) if not p.get("deleted")]

def get_variants():
    if USE_MONGO:
        return list(db.variants.find({"deleted": {"$ne": True}}))
    return [v for v in load_json("variants", {"items": []}).get("items", []) if not v.get("deleted")]

def get_colors():
    if USE_MONGO:
        return list(db.colors.find({"deleted": {"$ne": True}}))
    return [c for c in load_json("colors", {"items": []}).get("items", []) if not c.get("deleted")]
...
# ============================================================
# AI PRODUCT SEARCH — NV-CLIP image embeddings
# ============================================================
async def get_image_embedding(content: bytes, mime_type: str = "image/jpeg") -> list:
    """Call NVIDIA NV-CLIP to embed one image into a 1024-dim vector.
    Raises HTTPException on any failure so callers get a clear message."""
    if not NVIDIA_API_KEY:
        raise HTTPException(status_code=503, detail="AI belum dikonfigurasi: NVIDIA_API_KEY belum diset di server")
    import requests
    from starlette.concurrency import run_in_threadpool
    b64 = base64.b64encode(content).decode("ascii")
    data_url = f"data:{mime_type};base64,{b64}"
    payload = {"input": [data_url], "model": NVCLIP_MODEL, "encoding_format": "float"}
    headers = {"Authorization": f"Bearer {NVIDIA_API_KEY}", "Accept": "application/json"}
    try:
        response = await run_in_threadpool(requests.post, NVIDIA_EMBED_URL, json=payload, headers=headers, timeout=30)
    except requests.RequestException as exc:
        raise HTTPException(status_code=502, detail=f"Gagal menghubungi NVIDIA API: {exc}")
    if response.status_code != 200:
        raise HTTPException(status_code=502, detail=f"NVIDIA API error ({response.status_code}): {response.text[:200]}")
    body = response.json()
    try:
        return body["data"][0]["embedding"]
    except (KeyError, IndexError, TypeError):
        raise HTTPException(status_code=502, detail="Respons NVIDIA API tidak sesuai format yang diharapkan")
...
def set_product_embedding(product_id: str, vector: list):
    if USE_MONGO:
        db.products.update_one({"_id": oid(product_id)}, {"$set": {"image_embedding": vector}})
    else:
        data = load_json("products", {"items": []})
        found = next((p for p in data["items"] if str(p.get("id")) == str(product_id)), None)
        if found:
            found["image_embedding"] = vector
            save_json("products", data)

async def index_product_image(product_id: str, content: bytes, mime_type: str = "image/jpeg"):
    """Best-effort: compute + store an embedding for a product's image.
    Never raises — indexing failures must not block the actual image/product save."""
    if not NVIDIA_API_KEY:
        return
    try:
        vector = await get_image_embedding(content, mime_type)
        set_product_embedding(product_id, vector)
    except Exception as exc:
        print(f"AI indexing warning for product {product_id}: {exc}")

def read_product_image_bytes(product: dict):
    """Read a product's own stored image, from local /uploads or by fetching its Cloudinary URL."""
    import requests
...
async def ai_status():
    ...
    total = len(products)
    indexed = sum(1 for p in products if p.get("image_embedding"))
    return {"total": total, "indexed": indexed, "remaining": total - indexed, "configured": bool(NVIDIA_API_KEY)}

@app.post("/api/ai/reindex-batch")
async def ai_reindex_batch(batch_size: int = Query(8, ge=1, le=50)):
    if not NVIDIA_API_KEY:
        raise HTTPException(status_code=503, detail="AI belum dikonfigurasi: NVIDIA_API_KEY belum diset di server")
    products = get_products()
    total = len(products)
    pending = [p for p in products if not p.get("image_embedding")][:batch_size]
    processed = 0
    errors = []
    from starlette.concurrency import run_in_threadpool
    for product in pending:
        pid = str(product.get("_id", product.get("id")))
        content, mime_type = await run_in_threadpool(read_product_image_bytes, product)
        if not content:
            errors.append({"product_id": pid, "error": "Gambar produk tidak ditemukan/tidak bisa dibaca"})
            continue
        try:
            vector = await get_image_embedding(content, mime_type or "image/jpeg")
            set_product_embedding(pid, vector)
            processed += 1
        except HTTPException as exc:
            errors.append({"product_id": pid, "error": exc.detail})
        except Exception as exc:
            errors.append({"product_id": pid, "error": str(exc)})
    products_after = get_products()
    indexed_after = sum(1 for p in products_after if p.get("image_embedding"))
    return {
        "total": total,
        "indexed": indexed_after,
        "remaining": total - indexed_after,
        "processed": processed,
        "errors": errors,
    }

@app.post("/api/ai/search")
async def ai_search(image: UploadFile):
    if not NVIDIA_API_KEY:
        raise HTTPException(status_code=503, detail="AI belum dikonfigurasi: NVIDIA_API_KEY belum diset di server")
    ...
    query_vector = await get_image_embedding(content, image.content_type)

    products = [p for p in get_products() if p.get("image_embedding")]
    scored = []
    for product in products:
        score = cosine_similarity(query_vector, product["image_embedding"])
        scored.append((score, product))
    scored.sort(key=lambda pair: pair[0], reverse=True)

    def to_result(score, product):
        return {
    ...
    if not scored or scored[0][0] < AI_MIN_SCORE:
        return {"mode": "not_found", "results": []}

    top_score = scored[0][0]
    second_score = scored[1][0] if len(scored) > 1 else 0.0
    if top_score >= AI_CONFIDENT_SCORE and (top_score - second_score) >= AI_CONFIDENT_GAP:
        return {"mode": "confident", "results": [to_result(*scored[0])]}

    candidates = [pair for pair in scored[:AI_MAX_CANDIDATES] if pair[0] >= AI_MIN_SCORE]
    return {"mode": "ambiguous", "results": [to_result(score, product) for score, product in candidates]}
...
async def upload_product_image(product_id: str, image: UploadFile):
    ...
    await index_product_image(product_id, content, image.content_type)
